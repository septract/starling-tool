        ...
        [ testCase "Convert the empty CondView-list to the empty GuarView-list" 
          ...
          testCase "Convert a flat CondView-list to a GuarView-list with no guards" <| fun _ -> 
              Assert.Equal("[| foo(bar), bar(baz) |] = <| ([], foo(bar)), ([], bar(baz)) |>", 
                           Multiset.ofList [ { GCond = ctx.MkTrue()
                                               GItem = 
                                                   { VName = "foo"
                           ...
                           resolveCondViews ctx (Multiset.ofList [ CSetView { VName = "foo"
          ...
          <| fun _ -> 
              Assert.Equal
                  ("[| if s then foo(bar) else bar(baz) |] = <| (s, foo(bar)) (not s, bar(baz)) |>", 
                   Multiset.ofList [ { GCond = ctx.MkBoolConst "s"
                                       GItem = 
                                           { VName = "foo"
                                             VParams = [ ctx.MkIntConst "bar" :> Expr ] } }
                                     { GCond = ctx.MkNot(ctx.MkBoolConst "s")
                                       GItem = 
                                           { VName = "bar"
                                             VParams = [ ctx.MkIntConst "baz" :> Expr ] } } ], 
                   resolveCondViews ctx 
                       (Multiset.ofList 
                            [ CITEView((ctx.MkBoolConst "s"), 
                                       Multiset.ofList [ CSetView { VName = "foo"
                                                                    VParams = [ ctx.MkIntConst "bar" :> Expr ] } ], 
                                       Multiset.ofList [ CSetView { VName = "bar"
                                                                    VParams = [ ctx.MkIntConst "baz" :> Expr ] } ]) ]))
                   ...
                   resolveCondViews ctx 
                       (Multiset.ofList 
                            [ CITEView(ctx.MkBoolConst "s", 
                                       ...
                                       Multiset.ofList [ CSetView { VName = "ding"
