...
type ExpanderTests() =
    ...
    static member CondViews =
        seq {
            yield (
                new TestCaseData(Multiset.empty())
            ).Returns(
                Multiset.empty()
            ...
            yield (
                ...
                Multiset.ofList [ { Cond = BConst (Unmarked "s")
                                    Item = 
                                        { Name = "foo"
                                          Params = [ AExpr (AConst (Unmarked "bar")) ] } }
            ...
            yield (
                ...
                Multiset.ofList [ { Cond = BAnd [ BConst (Unmarked "s")
                                                  BConst (Unmarked "t") ]
                                    Item = 
                                        { Name = "foo"
                                          Params = [ AExpr (AConst (Unmarked "bar")) ] } }
                                  { Cond = BAnd [ BConst (Unmarked "s")
                                                  BConst (Unmarked "t") ]
                                    Item = 
                                        { Name = "bar"
                                          Params = [ AExpr (AConst (Unmarked "baz")) ] } }
    ...
    member x.``Test converting CondViews into GuarViews`` cv =
        resolveCondViews cv
