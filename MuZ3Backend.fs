...
module Run =
    ...
    let genViewDefRule reals (ctx : Z3.Context) fm (fixedpoint : Z3.Fixedpoint) (unsafeapp : Z3.BoolExpr) view def
      : Result<unit, Error> =
        let defImpliesViewResult =
            Translator.mkRule reals id ctx fm def Multiset.empty view
        let defImpliesViewAddResult : Result<unit, Error> =
            lift (Option.iter fixedpoint.AddRule) defImpliesViewResult
        ...
        let varsResult =
            mapMessages Traversal
                (lift2 Set.union defVarsResult paramVarsResult)
        ...
        lift2 (fun _ -> Option.iter fixedpoint.AddRule)
            defImpliesViewResult
            viewImpliesDefResult
