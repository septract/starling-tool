...
let mkAnd (ctx : Z3.Context) (conjuncts : Z3.BoolExpr seq) = 
    if Seq.exists (fun (x : Z3.BoolExpr) -> x.IsFalse) conjuncts then ctx.MkFalse()
    else 
        conjuncts
        |> Seq.filter (fun (x : Z3.BoolExpr) -> not x.IsTrue)
        |> Array.ofSeq
        |> function 
        | [||] -> ctx.MkTrue() // True is the zero of and.
        | [| x |] -> x
        | xs -> ctx.MkAnd(xs)
...
let mkOr (ctx : Z3.Context) (disjuncts : Z3.BoolExpr seq) = 
    if Seq.exists (fun (x : Z3.BoolExpr) -> x.IsTrue) disjuncts then ctx.MkTrue()
    else 
        disjuncts
        |> Seq.filter (fun (x : Z3.BoolExpr) -> not x.IsFalse)
        |> Array.ofSeq
        |> function 
        | [||] -> ctx.MkFalse() // False is the zero of or.
        | [| x |] -> x
        | xs -> ctx.MkOr(xs)
...
/// Makes not-equals.
let mkNeq (ctx : Z3.Context) l r = ctx.MkNot(ctx.MkEq(l, r))
