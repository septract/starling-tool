...
let testExpr (ctx : Context) = 
    ctx.MkAnd [| ctx.MkGt(ctx.MkIntConst "ticket!after", ctx.MkIntConst "ticket!before")
                 ctx.MkLe(ctx.MkIntConst "serving!after", ctx.MkIntConst "serving!after")
                 ctx.MkBoolConst "frozzle"
                 ctx.MkEq(ctx.MkIntConst "s!before", ctx.MkIntConst "t!before") |]
...
let testAftersInModel (ctx : Context) = 
    testList "test aftersInModel" 
        [ testCase "afters in the ticketed lock" <| fun _ -> 
              Assert.Equal("afters in ticketed lock = s!after, t!after, serving!after, ticket!after", 
...
let testFrame (ctx : Context) = 
    testList "test frame" 
        [ testCase "frame with the test expr and ticketed lock model" 
          <| fun _ -> 
              Assert.Equal
                  ("frame testExpr = <s!after=s!before, t!after=s!before>", 
                   [ ctx.MkEq(ctx.MkIntConst "s!after", ctx.MkIntConst "s!before")
                     ctx.MkEq(ctx.MkIntConst "t!after", ctx.MkIntConst "t!before") ], 
...
let testSemanticsOf (ctx : Context) = 
    testList "test semanticsOf" 
        [ testCase "semanticsOf with assume(s == t) and ticketed lock model" 
          <| fun _ -> 
              Assert.Equal
                  ("semanticsOf assume(s==t) <s!before = t!before; n!after = n!before>", 
                   // Annoyingly, order of terms seems to matter here.
                   // TODO(CaptainHayashi): weaken this test?
                   ctx.MkAnd [| ctx.MkEq(ctx.MkIntConst "serving!after", ctx.MkIntConst "serving!before")
                                ctx.MkEq(ctx.MkIntConst "ticket!after", ctx.MkIntConst "ticket!before")
                                ctx.MkEq(ctx.MkIntConst "s!after", ctx.MkIntConst "s!before")
                                ctx.MkEq(ctx.MkIntConst "t!after", ctx.MkIntConst "t!before")
                                ctx.MkEq(ctx.MkIntConst "s!before", ctx.MkIntConst "t!before") |], 
                   semanticsOf (ticketLockModel ctx) (PrimAssume(ctx.MkEq(ctx.MkIntConst "s", ctx.MkIntConst "t"))))
          
          testCase "semanticsOf with serving++ and ticketed lock model" 
          <| fun _ -> 
              Assert.Equal
                  ("semanticsOf serving++ = <serving!after = serving!before + 1; n!after = n!before>", 
                   // Annoyingly, order of terms seems to matter here.
                   // TODO(CaptainHayashi): weaken this test?
                   ctx.MkAnd [| ctx.MkEq(ctx.MkIntConst "ticket!after", ctx.MkIntConst "ticket!before")
                                ctx.MkEq(ctx.MkIntConst "s!after", ctx.MkIntConst "s!before")
                                ctx.MkEq(ctx.MkIntConst "t!after", ctx.MkIntConst "t!before")
                                ctx.MkEq(ctx.MkIntConst "serving!after", 
                                         ctx.MkAdd [| ctx.MkIntConst "serving!before" :> ArithExpr
                                                      ctx.MkInt 1 :> ArithExpr |]) |], 
                   semanticsOf (ticketLockModel ctx) (IntLoad(None, LVIdent "serving", Increment))) ]
...
let testSemantics = 
    ...
    let t = 
        testList "Test the command semantics translation" [ testExprsInExpr ctx
